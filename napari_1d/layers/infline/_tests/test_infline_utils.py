...
from napari_1d.layers.infline._infline_utils import parse_inf_line_orientation


def test_none():
    data, orientations = parse_inf_line_orientation(None)
...
def test_tuple_single(orientation):
    data, orientations = parse_inf_line_orientation((50, orientation))
    assert len(data) == len(orientations)
...
def test_list_of_tuples(orientation):
    ...
    data, orientations = parse_inf_line_orientation(inputs)
    assert len(data) == len(orientations)


@pytest.mark.parametrize("orientation", ("vertical", Orientation.VERTICAL))
def test_array_same_orientation(orientation):
    data, orientations = parse_inf_line_orientation(np.arange(10), orientation)
    assert len(data) == len(orientations)
