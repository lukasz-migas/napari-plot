...
@pytest.mark.skipif(bool(os.environ.get("MIN_REQ")), reason="skip import time test on MIN_REQ")
def test_import_time(tmp_path):
    cmd = [sys.executable, "-X", "importtime", "-c", "import napari_plot"]
    ...
    print(f"\nnapari-plot took {int(time)/1e6:0.3f} seconds to import")
