...
class VispyScatterLayer(VispyBaseLayer):
    ...
    def _on_data_change(self):
        if len(self.layer._indices_view) > 0:
            edge_color = self.layer._view_border_color
        ...
        if len(self.layer._indices_view) == 0:
            ...
            edge_width = self.layer._view_border_width
            symbol = [str(x) for x in self.layer._view_symbol]

        if self.layer.border_width_is_relative:
            border_kw = {"edge_width": None, "edge_width_rel": edge_width}
        else:
            border_kw = {"edge_width": edge_width, "edge_width_rel": None}

        self.node.points_markers.set_data(
            data[:, ::-1],
            size=size,
            symbol=symbol,
